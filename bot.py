...
from scipy.stats import entropy, ks_2samp, anderson, mannwhitneyu  
...
async def init_influx():
    ...
    try:
        ...
        # Test write: Send dummy point to verify write path
        ...
        logger.error(f"InfluxDB init failed: {reason}")
        return False

async def write_to_influx(measurement: str, fields: Optional[Dict] = None, tags: Optional[Dict] = None):
    """Async write to InfluxDB with enhanced error handling and logging."""
    ...
    tags = tags or {}
    try:
        logger.debug(f"Attempting to write to InfluxDB: measurement={measurement}, fields={fields}, tags={tags}")
        point = Point(measurement)
        if tags:
            for k, v in tags.items():
                point.tag(k, str(v))  # Ensure tags as strings (best practice)
        for k, v in fields.items():
            fields = {k: v for k, v in (fields or {}).items() if isinstance(v, (int, float, str))}
            fields = {k: (0.0 if np.isnan(v) or np.isinf(v) else v) if isinstance(v, float) else v for k, v in fields.items()}
            if isinstance(v, (int, float)):
                point.field(k, float(v))  # Ensure numeric fields as float
            else:
                point.field(k, str(v))  # Strings as string
        point.time(datetime.now(timezone.utc), WritePrecision.NS)        
        
        write_api.write(bucket=INFLUXDB_BUCKET, org=INFLUXDB_ORG, record=point)        
        
        logger.debug(f"Successfully wrote to InfluxDB: measurement={measurement}")
   
    except Exception as e:
        reason = str(e)
        if "authentication" in reason.lower():
            reason += " (Invalid token - check INFLUXDB_TOKEN)"
        elif "not found" in reason.lower():
            reason += " (Bucket/Org not found - check INFLUXDB_BUCKET/ORG)"
        elif "connection" in reason.lower():
            reason += " (Network issue - check INFLUXDB_URL)"
        else:
            reason += " (Unknown error)"
        logger.error(f"InfluxDB write failed for {measurement}: {reason}", exc_info=True)
        # No data sent: Log explicitly
        logger.error(f"No data reached InfluxDB for {measurement}: {reason}")
...
FEATURE_COLS = [
    ...
    'mean_reversion_signal', 'body_shadow_ratio'
]
...
METRICS_LOCK_SYNC = asyncio.Lock()  
...
class VolRegDet:
    def __init__(self, exchange: ExchIntf):
        self.regime_cache = {}
...
class DbIntf:
    ...
    async def save_performance_metrics(self, metrics: Dict):
        ...
        try:
            ...
            logger.error(f"Error saving metrics to InfluxDB: {e}")
...
class QNetwork(nn.Module):
    def __init__(self, state_dim: int, hidden_dims: List[int] = [64, 32], action_dim: int = 18):
        super().__init__()
        layers = []
        prev_dim = state_dim
        ...
        layers.append(nn.Linear(prev_dim, action_dim))  # Dynamic action_dim
...
class ParamOpt:
    ...
    def get_rl_proposal(self, metrics: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            if len(self.replay_buffer) < 32:  # Minimum buffer for stable training:
...
# New: Meta-RL for intervention decisions
class MetaRLNetwork(QNetwork):
    def __init__(self, base_state_dim: int, hidden_dims: List[int] = [64, 32]):
        # base_state_dim is optimizer.state_dim (9); super gets 10 (for missed_count slot)
        super().__init__(base_state_dim + 1, hidden_dims, action_dim=2)
        self.action_dim = 2  # 0: no intervene, 1: intervene

    def get_intervene_prob(self, state: np.ndarray, missed_count: int) -> float:
        full_state = np.append(state, missed_count / 10.0)  # Normalize missed (9 + 1 = 10)
        with torch.no_grad():
            q_values = self.forward(torch.tensor(full_state, dtype=torch.float32).unsqueeze(0))
            probs = torch.softmax(q_values, dim=-1)
            return probs[0, 1].item()  # Prob of intervene (action 1)

class SigGen:
    def __init__(self, exchange: ExchIntf):
        self.min_data_points = 50
        self.regime_det = VolRegDet(exchange)
    ...
    async def generate_signal(self, df: pd.DataFrame, tf_config: Dict[str, Any], symbol: str = "", historical: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            'timestamp': time.time(), 'atr': atr, 'df': df
        }
...
class BtMgr:
    def __init__(self, exchange: ExchIntf, db: DbIntf, sig_gen: SigGen, model_mgr: ModelMgr, param_opt: ParamOpt, regime_det: VolRegDet):
        self.exchange = exchange
        self.db = db
    ...
    async def get_150_pairs(self) -> Tuple[List[str], Dict[str, List[str]]]:
        try:
            symbols = [s for s in self.exchange.markets if self.exchange.markets[s].get('spot', False) and s.endswith('/USDT') and self.exchange.markets[s]['active'] and self.exchange.markets[s]['base'] not in STABLES]
            sem = asyncio.Semaphore(10)
    ...
    async def run_backtest_on_pair(self, symbol: str, initial_equity: float = 10000.0) -> Dict:
        ...
        CLOSED_TRADES_SINCE_LAST = 0
        bt_logger.info(f"Backtest complete: Reset CLOSED_TRADES_SINCE_LAST to 0")
    ...
    def _estimate_enhanced_garch(self, returns: np.ndarray, regime_params: Dict) -> Dict:
        ...
        persistence = alpha + beta
        if persistence >= 0.999:
            bt_logger.warning(f"GARCH non-stationary for regime {regime} (persistence={persistence:.4f}), applying correction")
            # Escalar para forzar estacionariedad
            scale_factor = 0.98 / persistence
            alpha *= scale_factor
            beta *= scale_factor
            persistence = alpha + beta
    ...
    def _analyze_market_microstructure(self, df: pd.DataFrame, regime_params: Dict) -> Dict:
        """
        Análisis de microestructura considerando whale_risk y otros indicadores
        """
        analysis = {
    ...
    async def _advanced_statistical_validation(self, historical_returns: np.ndarray, 
                                        paths_ensemble: np.ndarray, symbol: str, 
                                        regime: str, timeframe_params: Dict) -> Dict:
        """
        Validación estadística avanzada con diagnósticos mejorados
        """
        # Aplanar paths para análisis
        if paths_ensemble is None or paths_ensemble.size == 0:
            bt_logger.warning(f"MC Validation: Empty/None paths_ensemble for {symbol}; neutral scores")
            return {'composite_score': 0, 'ks_p_value': 0, 'anderson_p': 0, 'autocorr_score': 0}
        flat_sim = paths_ensemble.flatten()
        original_size = len(flat_sim)
        flat_sim = flat_sim[~np.isnan(flat_sim)]  # Remover NaN
        clean_size = len(flat_sim)
        
        # DIAGNÓSTICO 1: Verificar datos
        bt_logger.debug(f"MC Validation for {symbol}: Original size={original_size}, Clean size={clean_size}")
        bt_logger.debug(f"Historical returns: mean={np.mean(historical_returns):.6f}, std={np.std(historical_returns):.6f}")
        
        if len(flat_sim) == 0:
            bt_logger.warning(f"MC Validation failed: No valid simulation data for {symbol}")
            return {'composite_score': 0, 'ks_p_value': 0, 'anderson_p': 0, 'autocorr_score': 0}
        
        # Convertir a returns para comparación correcta
        if len(flat_sim) > 1:
            sim_returns = np.diff(flat_sim) / flat_sim[:-1]
            sim_returns = sim_returns[~np.isnan(sim_returns)]
        else:
            bt_logger.warning(f"MC Validation: Insufficient simulation data for {symbol}")
            return {'composite_score': 0, 'ks_p_value': 0, 'anderson_p': 0, 'autocorr_score': 0}
        bt_logger.debug(f"Simulation returns: mean={np.mean(sim_returns):.6f}, std={np.std(sim_returns):.6f}")
        validation_results = {}
        # Test 1: Kolmogorov-Smirnov - CORREGIDO (use two-sample KS for better fit to historical)
        try:
            from scipy.stats import ks_2samp, norm, anderson, skew, kurtosis, mannwhitneyu
            
            # Two-sample KS for direct comparison to historical (improves p-value by testing similarity)
            if len(historical_returns) > 0 and len(sim_returns) > 0:
                # FIXED: Truncate to min length for broadcast-safe (prevents shape mismatch)
                min_len = min(len(sim_returns), len(historical_returns))
                sim_trunc = sim_returns[:min_len]
                hist_trunc = historical_returns[:min_len]
                ks_stat, ks_p = ks_2samp(sim_trunc, hist_trunc)
            else:
                ks_p = 0.0
            
            # Fallback to Mann-Whitney U if KS p too low (non-parametric rank test for distribution shift)
            if ks_p < 0.01:
                _, mw_p = mannwhitneyu(sim_returns, historical_returns, alternative='two-sided')
                ks_p = max(ks_p, mw_p * 0.5)  # Blend for conservatism
                bt_logger.debug(f"Low KS p-value ({ks_p:.4f}), using Mann-Whitney fallback (p={mw_p:.4f})")
            
            validation_results['ks_p_value'] = ks_p
            bt_logger.debug(f"KS test: stat={ks_stat:.4f}, p={ks_p:.4f}")
            
        except Exception as e:
            bt_logger.warning(f"KS test failed for {symbol}: {e}")
            validation_results['ks_p_value'] = 0        
      
        # Test 2: Anderson-Darling - CORREGIDO
        try:
            # Normalize sim_returns for normality test
            sim_mean = np.mean(sim_returns)
            sim_std = np.std(sim_returns)
            if sim_std > 0:
                sim_norm = (sim_returns - sim_mean) / sim_std
            else:
                sim_norm = np.zeros_like(sim_returns)  # Fallback to zeros if no variance
            # Limit size and handle small samples
            sample_size = min(1000, len(sim_norm))
            if sample_size < 5:  # Min for anderson
                raise ValueError("Sample too small for Anderson-Darling")
            ad_result = anderson(sim_norm[:sample_size], dist='norm')
            ad_stat = ad_result.statistic
            # Safe p-value approximation (anderson returns critical values, not p)
            p_approx = np.mean([1 if ad_stat < crit else 0 for crit in ad_result.critical_values])  # Fraction below critical
            validation_results['anderson_p'] = max(0.001, min(0.999, p_approx))
            bt_logger.debug(f"Anderson test: stat={ad_stat:.4f}, p={validation_results['anderson_p']:.4f}")
            
        except ValueError as ve:
            bt_logger.warning(f"Anderson test failed for {symbol}: {ve} (small sample)")
            validation_results['anderson_p'] = 0.5
        except Exception as e:
            
            bt_logger.warning(f"Anderson test failed for {symbol}: {e}")
            validation_results['anderson_p'] = 0.5
        # Test 3: Comparación de momentos - MEJORADO
        def moments_test(hist, sim):
            try:
                # Usar solo primeros 4 momentos
                hist_moments = [np.mean(hist), np.std(hist), skew(hist), kurtosis(hist)]
                sim_moments = [np.mean(sim), np.std(sim), skew(sim), kurtosis(sim)]
                
                moment_scores = []
                for i, (h_mom, s_mom) in enumerate(zip(hist_moments, sim_moments)):
                    if abs(h_mom) > 1e-10:
                        diff = abs(h_mom - s_mom) / abs(h_mom)
                    else:
                        diff = abs(s_mom)
                    
                    # Ponderación: media y std más importantes
                    weight = [2.0, 2.0, 1.0, 0.5][i]
                    score = max(0, 1 - diff) * weight
                    moment_scores.append(score)
                    
                    bt_logger.debug(f"Moment {i}: hist={h_mom:.4f}, sim={s_mom:.4f}, score={score:.4f}")
                
                final_score = np.average(moment_scores, weights=[2.0, 2.0, 1.0, 0.5])
                return max(0, min(1, final_score))
                
            except Exception as e:
                bt_logger.warning(f"Moments test failed: {e}")
                return 0.5
        
        validation_results['moments_score'] = moments_test(historical_returns, sim_returns)
        
        # Test 4: Autocorrelación - MEJORADO
        def timeframe_autocorr_test(hist, sim, timeframe_params):
            try:
                strategy = timeframe_params.get('strategy_type', 'trend_following')
                
                if strategy == 'micro_scalp':
                    lags = range(1, min(4, len(hist)//10))
                elif strategy == 'trend_following':
                    lags = range(1, min(8, len(hist)//5))
                else:
                    lags = range(1, min(6, len(hist)//8))
                
                valid_scores = []
                
                for lag in lags:
                    if len(hist) > lag*2 and len(sim) > lag*2:
                        try:
                            hist_corr = np.corrcoef(hist[:-lag], hist[lag:])[0, 1]
                            sim_corr = np.corrcoef(sim[:-lag], sim[lag:])[0, 1]
                            
                            if not (np.isnan(hist_corr) or np.isnan(sim_corr)):
                                diff = abs(hist_corr - sim_corr)
                                score = max(0, 1 - diff * 2)  # Menos penalización
                ...
                return 0.6  # Valor neutro si no se puede calcular
                
            except Exception as e:
                bt_logger.warning(f"Autocorr test failed: {e}")
                return 0.6
        
        validation_results['autocorr_score'] = timeframe_autocorr_test(
            historical_returns, sim_returns, timeframe_params
        )
        
        # Score compuesto MEJORADO - más tolerante
        weights = {
            'ks_p_value': 0.25,      # Reducido
            'anderson_p': 0.20,      # Reducido  
            'moments_score': 0.35,   # Aumentado
            'autocorr_score': 0.20   # Aumentado
        }
        
        # Normalización más tolerante
        ks_norm = min(1.0, max(0.001, validation_results['ks_p_value'] * 50))  # Más tolerante
        anderson_norm = min(1.0, max(0.1, validation_results['anderson_p'] * 5))  # Más tolerante
        
        composite_score = (
            weights['ks_p_value'] * ks_norm +
            weights['anderson_p'] * anderson_norm +
            weights['moments_score'] * validation_results['moments_score'] +
            weights['autocorr_score'] * validation_results['autocorr_score']
        )
        
        validation_results['composite_score'] = max(0.1, composite_score)  # Mínimo 0.1
        
        # Log detallado
        bt_logger.info(f"MC Validation {symbol}: KS={ks_norm:.3f}, Anderson={anderson_norm:.3f}, "
                       f"Moments={validation_results['moments_score']:.3f}, "
                       f"Autocorr={validation_results['autocorr_score']:.3f}, "
                       f"Composite={validation_results['composite_score']:.3f}")
        
        return validation_results

    def _calculate_risk_adjusted_metrics(self, paths: np.ndarray, last_price: float,
                                   metrics: Dict, validation_results: Dict,
                                   regime_params: Dict) -> Dict:
        """
        Métricas finales con ajustes por calidad y régimen
        """
        ...
        pnl = metrics.get('realized_pnl', 0)
        win_rate = metrics.get('win_rate', 0.5)
        validation_quality = validation_results.get('composite_score', 0.5)  # ✅ Usar .get() con fallback
        
        # Ajuste inteligente por win_rate
        win_rate_factor = max(0.1, min(2.0, win_rate / 0.5)) if win_rate > 0 else 0.5
        
        # Ajuste por calidad de simulación (más conservador)
        quality_factor = 0.3 + 0.7 * validation_quality
        
        # Ajuste por régimen de mercado
        regime_risk_factor = {
            'very_low': 0.9, 'low': 0.95, 'normal': 1.0,
            'bull': 1.05, 'bear': 0.85, 'high': 0.9,
            'volatile_bull': 0.8, 'volatile_bear': 0.7
        }.get(regime_params.get('regime', 'normal'), 1.0)  # ✅ Usar .get() anidado
        
        # PnL simulado final
        simulated_pnls = path_total_returns * pnl * win_rate_factor * quality_factor * regime_risk_factor
        
        if len(simulated_pnls) < 10:  # Min for reliable percentiles
            return {'pnl_5th': 0, 'pnl_50th': 0, 'pnl_95th': 0, 'validation_score': validation_quality}
        # Ensure scalar floats for production logging
        ...
        q_values = np.array([5, 25, 50, 75, 95])  # q as array for multi-percentile
        pnl_percentiles = np.percentile(simulated_pnls, q_values)

        # Métricas adicionales de market risk (asegurar escalares)
        var_95 = float(np.percentile(simulated_pnls, 5))  # Value at Risk como float scalar
        cvar_mask = simulated_pnls <= var_95
        cvar_95 = float(np.mean(simulated_pnls[cvar_mask])) if np.sum(cvar_mask) > 0 else float(var_95)
        
        # Sharpe ratio simulado
        std_pnl = np.std(simulated_pnls)
        if std_pnl > 0:
            simulated_sharpe = np.mean(simulated_pnls) / std_pnl
        else:
            simulated_sharpe = 0.0  # Explicit float
        
        # Máximo drawdown simulado
        if len(simulated_pnls) > 0:
            cumulative_pnls = np.cumsum(simulated_pnls.reshape(-1, 1))
            running_max = np.maximum.accumulate(cumulative_pnls, axis=0)
            drawdowns = (cumulative_pnls - running_max) / np.maximum(running_max, 1e-8)  # Evitar /0 con epsilon
            max_drawdown = np.min(drawdowns.flatten()) * 100 if len(drawdowns) > 0 else 0.0
        else:
            max_drawdown = 0.0
        ...
        final_metrics = {
            ...
            'max_drawdown_sim': max_drawdown,
            'validation_score': validation_quality,
            'ks_p_value': validation_results.get('ks_p_value', 0),
            'anderson_p_value': validation_results.get('anderson_p', 0),
            'paths_generated': len(paths),
            'regime_applied': regime_params.get('regime', 'unknown'),
            'trap_detected': regime_params.get('trap_detected', False)
        }
        
        # ✅ Usar configuración directa ya que existe en CONFIG
        mc_quality_threshold = CONFIG['default']['mc_quality_good_threshold']
        if final_metrics['validation_score'] < mc_quality_threshold:
            self.dynamic_mc_paths = min(5000, int(self.dynamic_mc_paths * 1.2))
            bt_logger.info(f"Adapted MC paths to {self.dynamic_mc_paths} for better quality")
...
class RiskMgr:
    def __init__(self, exchange: ExchIntf, bt_mgr: Optional[BtMgr] = None):
        self.exchange = exchange
        self.bt_mgr = bt_mgr
    ...
    async def validate_trade(self, symbol: str, side: str, quantity: float, price: float, confidence: float, current_positions: List[Dict], portfolio_value: float, df: pd.DataFrame = None, state: dict = None) -> Dict[str, Any]:
        ...
        validation = {'approved': False, 'reason': '', 'adjusted_quantity': quantity, 'risk_score': 0.0, 'position_limit_ok': True, 'size_limit_ok': True, 'confidence_ok': True, 'portfolio_risk_ok': True, 'correlation_ok': True}
        ...
        validation.update({'approved': True, 'reason': 'Approved', 'risk_score': sum(risk_factors), 'total_risk': total_risk})
...
class SupBot:
    def __init__(self, optimizer: ParamOpt, bt_mgr: BtMgr):
...
class TradeExec:
    def __init__(self, exchange: ExchIntf, db: DbIntf, risk_mgr: RiskMgr, bt_mgr: BtMgr, supervisor_bot: SupBot):
        self.exchange = exchange
        self.db = db
    ...
    async def execute_signal(self, signal: Dict, tf_config: Dict, symbol: str, portfolio_value: float) -> Optional[Dict]:
        ...
        if signal['direction'] == 'BUY' and PERFORMANCE_METRICS.get('profitable_trades', 0) / max(1, PERFORMANCE_METRICS.get('total_closed_trades', 1)) > 0.6:
            regime_key = regime if regime in CONFIG else 'default'
            CONFIG[regime_key].setdefault('kelly_frac', CONFIG['default']['kelly_frac'])
            CONFIG[regime_key]['kelly_frac'] = min(1.0, CONFIG[regime_key]['kelly_frac'] + 0.05)
...
async def auto_trade_loop(exchange: ExchIntf, db: DbIntf):
    ...
    async with METRICS_LOCK_ASYNC:
        ...
        last_top_fetch = last_top_fetch or 0
        last_supervisor_run = last_supervisor_run or 0
        missed_analysis = missed_analysis or []
        CLOSED_TRADES_SINCE_LAST = 0
        symbols = await detect_tradeable_symbols(exchange, trade_exec, regime=state['regime_global'])  # Single calc, rename to symbols
    ...
    async def train_symbol(s, tf_cfg):
        df = await fetch_and_prepare_data(s, tf_cfg, exchange, state)  # FIXED: Pass state consistently
        if df is not None:
        ...
        while True:
            ...
            if iteration % 5 == 0 or state.get('consecutive_failures', 0) > 2:
                ...
                try:
                    ...
                    state['consecutive_failures'] += 1
                    if state['consecutive_failures'] > 5:  # Threshold for aggressive reconnect
            ...
            if iteration % 5 == 1:
                if bt_mgr.top_performers and len(symbols) < len(bt_mgr.top_performers):
                    # Agrega solo nuevos rentables (evita duplicados)
            ...
            if now - last_top_fetch >= get_config_param('perf_check_int'):                  
                ...
                logger.debug("Skipping top performers fetch this cycle; using previous missed analysis")
