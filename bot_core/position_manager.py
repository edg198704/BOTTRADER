...
class PositionManager:
    ...
    def _ensure_schema_updates(self):
        # Schema migration logic omitted for brevity, assuming fresh DB or compatible schema
        pass
    ...
    async def reconcile_positions(self, exchange_api: 'ExchangeAPI', latest_prices: Dict[str, float]):
        ...
        for symbol, pos in db_map.items():
            ...
            if held_balance < (pos.quantity * Decimal("0.95")):
            ...
            elif held_balance < (pos.quantity * Decimal("0.99")):
    ...
    def _get_portfolio_state_sync(self) -> Optional[Dict[str, Decimal]]:
        session = self.SessionLocal()
        try:
            state = session.query(PortfolioState).filter(PortfolioState.id == 1).first()
            if state:
    ...
    async def get_aggregated_open_positions(self) -> Dict[str, Decimal]:
        agg = {}
        for p in self._position_cache.values():
            if p.status == PositionStatus.OPEN:
                agg[p.symbol] = agg.get(p.symbol, ZERO) + p.quantity
        return agg
    ...
    def _create_pending_position_sync(self, symbol: str, side: str, order_id: str, trade_id: str, decision_price: Decimal, strategy_metadata: Optional[Dict[str, Any]]) -> Optional[Position]:
        session = self.SessionLocal()
        try:
            ...
            if updated_pos:
                ...
                if self.alert_system:
                    asyncio.run_coroutine_threadsafe(self.alert_system.send_alert(
                        ...
                        details={'symbol': symbol, 'side': updated_pos.side, 'quantity': str(quantity), 'entry_price': str(entry_price), 'fees': str(fees)}
                    ), asyncio.get_running_loop())
            return updated_pos

    def _confirm_position_open_sync(self, symbol: str, order_id: str, quantity: Decimal, entry_price: Decimal, stop_loss: Decimal, take_profit: Decimal, fees: Decimal) -> Optional[Position]:
        session = self.SessionLocal()
        try:
            position = session.query(Position).filter(
            ...
            if position.creation_timestamp:
                latency = to_decimal((fill_time - position.creation_timestamp).total_seconds() * 1000.0)
    ...
    def _confirm_position_close_sync(self, symbol: str, exit_price: Decimal, filled_qty: Decimal, fees: Decimal, reason: str) -> Optional[Position]:
        session = self.SessionLocal()
        try:
            position = session.query(Position).filter(Position.symbol == symbol, Position.status == PositionStatus.OPEN).first()
            if not position:
            ...
            if filled_qty < (position.quantity * Decimal("0.999")):
                # Partial Close
    ...
    def _close_position_sync(self, symbol: str, close_price: Decimal, reason: str, actual_filled_qty: Optional[Decimal], fees: Decimal) -> Optional[Position]:
        session = self.SessionLocal()
        try:
            position = session.query(Position).filter(Position.symbol == symbol, Position.status == PositionStatus.OPEN).first()
            if not position:
    ...
    def _update_position_stop_loss_sync(self, symbol: str, new_stop_price: Optional[Decimal], new_ref_price: Optional[Decimal], activate_trailing: bool) -> Optional[Position]:
        session = self.SessionLocal()
        try:
    ...
    def _activate_breakeven_sync(self, symbol: str, new_sl: Decimal) -> Optional[Position]:
        session = self.SessionLocal()
        try:
            pos = session.query(Position).filter(Position.symbol == symbol, Position.status == PositionStatus.OPEN).first()
            if pos:
    ...
    def get_portfolio_value(self, latest_prices: Dict[str, float], open_positions: List[Position]) -> Decimal:
        ...
        for pos in open_positions:
            # Convert latest price to Decimal for calculation
