    ...
    async def _handle_signal(self, signal: Dict):
        ...
        if action == 'BUY' and not position:
            ...
            if order_result and order_result.get('orderId'):
                ...
                closed_pos = self.position_manager.get_all_open_positions() # A bit of a hack to get last closed PnL
                # A better implementation would return the closed position from close_position
                # For now, we assume the last closed one is what we need.
                # This part needs improvement in PositionManager.
