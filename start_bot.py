...
def validate_config(config: BotConfig):
    ...
    if isinstance(config.strategy.params, AIEnsembleStrategyParams):
        ...
        # --- Whitelist Dynamic Features from FeatureProcessor ---
        ...
        if missing_columns:
            error_msg = (
                ...
                f"generated by the configured indicators or feature processor. Missing columns: {list(missing_columns)}"
            )
            logger.critical(error_msg, required=list(required_columns), available=list(available_columns))
            raise ValueError(error_msg)
        
        logger.info("AI feature columns are valid and can be generated by DataHandler/FeatureProcessor.")
